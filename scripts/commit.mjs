...
async function selectBranch(message) {
  ...
  return branchAnswer.branch === 'custom' ? branchAnswer.customBranch : branchAnswer.branch
}
...
async function selectCommit(message, ref = 'HEAD') {
  ...
  const answer = await inquirer.prompt([{ type: 'select', name: 'hash', message, choices }])
  return answer.hash
}
...
async function selectCommits(message, ref = 'HEAD') {
  ...
  const answer = await inquirer.prompt([{ type: 'checkbox', name: 'hashes', message, choices }])
...
async function createCommit() {
  ...
  const answers = await inquirer.prompt([
    ...
    { type: 'select', name: 'scope', message: 'Select workspace:', choices: [...scopes, 'custom'] },
    {
      type: 'input',
      name: 'customScope',
      message: 'Enter custom workspace name:',
      when: (a) => a.scope === 'custom',
    },
  ...
  const checks = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'runChecks',
      message: 'Run format and lint before commit?',
      default: true,
    },
  ])
